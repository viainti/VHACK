...
datasetRoutes.get('/export', (req, res) => {
  ...
  if (format === 'jsonl') {
    ...
    res.setHeader('Content-Disposition', 'attachment; filename="VHACK-dataset.jsonl"')
    const lines = dataset.map(entry => JSON.stringify(entry))
    res.send(lines.join('\n') + '\n')
  } else {
    // Standard JSON array
    res.setHeader('Content-Type', 'application/json')
    res.setHeader('Content-Disposition', 'attachment; filename="VHACK-dataset.json"')
    res.json({
      metadata: {
        ...
        license: 'AGPL-3.0',
        source: 'https://huggingface.co/datasets/pliny-the-prompter/VHACK',
