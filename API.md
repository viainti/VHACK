# VHACK Research Preview API

REST API exposing the **ULTRAPLINIAN** multi-model racing engine and core VHACK systems: **AutoTune** (context-adaptive LLM parameter tuning), **Parseltongue** (text obfuscation), **STM** (semantic text transformation), **Feedback Loop** (EMA-based parameter learning), and **opt-in open dataset collection**.
...
# Docker
docker build -t VHACK-api .
docker run -p 7860:7860 VHACK-api

# With auth enabled
docker run -p 7860:7860 -e GODMODE_API_KEY=your-secret-key VHACK-api
```
...
const completion = await client.chat.completions.create({
  model: 'nousresearch/hermes-3-llama-3.1-70b',
  messages: [{ role: 'user', content: 'Hello!' }],
  // @ts-ignore — VHACK extension field
...
The VHACK pipeline (GODMODE, AutoTune, Parseltongue, STM) runs transparently behind the standard interface. Pipeline metadata is returned in the `x_VHACK` extension field (ignored by standard SDKs). To disable the pipeline, pass `godmode: false, autotune: false, parseltongue: false, stm_modules: []`.
...
The response is standard OpenAI format. The winning model name is in `response.model`. Race metadata is in `x_VHACK.race`.
...
# x_VHACK.orchestrator.model → the synthesizer (e.g. "anthropic/claude-sonnet-4")
```
...
```json
{
  "object": "list",
  "data": [
    {"id": "ultraplinian/fast", "object": "model", "created": 1700000000, "owned_by": "VHACK"},
    {"id": "ultraplinian/standard", "object": "model", "created": 1700000000, "owned_by": "VHACK"},
    {"id": "ultraplinian/full", "object": "model", "created": 1700000000, "owned_by": "VHACK"},
...
| `godmode` | bool | no | `true` | Inject GODMODE system prompt (VHACK extension) |
| `autotune` | bool | no | `true` | Enable AutoTune (VHACK extension) |
| `parseltongue` | bool | no | `true` | Enable Parseltongue (VHACK extension) |
| `stm_modules` | array | no | `["hedge_reducer", "direct_mode"]` | STM modules (VHACK extension) |
| `contribute_to_dataset` | bool | no | `false` | Opt in to dataset (VHACK extension) |
...
huggingface-cli upload pliny-the-prompter/VHACK dataset.jsonl
```
...
Read-access endpoints for the [`pliny-the-prompter/VHACK`](https://huggingface.co/datasets/pliny-the-prompter/VHACK) HuggingFace dataset. Query, filter, and download the full published corpus — not just the current in-memory buffer.
...
with open("VHACK-dataset.jsonl", "w") as f:
    f.write(r.text)
...
with open("VHACK-corpus.jsonl", "wb") as f:
...
| `HF_DATASET_REPO` | Target HF dataset repo | `pliny-the-prompter/VHACK` |
