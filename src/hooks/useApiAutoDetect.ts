...
/**
 * Auto-detect a self-hosted VHACK API server at the same origin.
 *
    ...
    async function detect() {
      try {
        ...
        if (tierRes.ok || tierRes.status === 429) {
          ...
          console.log('[VHACK] Self-hosted API detected (open auth) at', origin)
          ...
          setUltraplinianApiUrl(origin)
          console.log('[VHACK] Self-hosted API detected (auth required) at', origin)
        }
      } catch {
