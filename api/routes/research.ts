 ...
 * Research API Routes
 *
 * Read-access endpoints for the pliny-the-prompter/VHACK HuggingFace
...
researchRoutes.get('/info', (_req, res) => {
  res.json({
    name: 'VHACK Research Dataset',
    repo: 'pliny-the-prompter/VHACK',
    repo_url: 'https://huggingface.co/datasets/pliny-the-prompter/VHACK',
    license: 'AGPL-3.0',
...
researchRoutes.get('/download', tierGate('corpus:download'), async (req, res) => {
  try {
    ...
    res.setHeader('Content-Disposition', `attachment; filename="VHACK-corpus-${Date.now()}.jsonl"`)
